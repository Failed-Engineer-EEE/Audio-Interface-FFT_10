...
class AudioEMCGUI:
    ...
    def create_widgets(self):
        ...
        tk.Label(col1, text="FFT Size (Power of 2):", font=('Arial', 10)).pack(anchor=tk.W, pady=(8,0))
        ...
        tk.Button(col1, text="Apply Graph Limits", command=self.apply_limits, bg="#b2dfdb", font=('Arial', 10, 'bold')).pack(fill=tk.X, pady=10)

        tk.Frame(col1, height=2, bd=1, relief=tk.SUNKEN).pack(fill=tk.X, pady=15)

        # 3. Execution & Export
        tk.Label(col1, text="3. Execution & Export", font=('Arial', 12, 'bold')).pack(anchor=tk.W)
        tk.Button(col1, text="Single Sweep", font=('Arial', 11, 'bold'), command=self.single_sweep, bg="#d1e7dd").pack(fill=tk.X, pady=6)
        ...
        # Make the graph visually update if the unit changes while stopped
        self.unit_combo.bind("<<ComboboxSelected>>", lambda e: self.force_redraw())
        ...
        tk.Button(col2, text="Load Limit File", command=self.load_limit_file, bg="#e0f2f1").pack(fill=tk.X, pady=4)
    ...
    def load_correction_file(self):
        filepath = filedialog.askopenfilename(title="Select Probe File", filetypes=[("EMC Files", "*.csv *.cor *.txt *.lsc")])
        if not filepath: return
        try:
            freqs, factors = [], []
            with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
    ...
    def fetch_and_plot(self, stream_mode=False):
        ...
        try:
            ...
            # Math Optimization: Pure NumPy Hanning Window
            n = len(audio_array)
            ...
            # Corrections (Probe/Antenna factors)
            ...
            # --- UNIT CONVERSION MATH ENGINE ---
            target_unit = self.unit_combo.get()
            
            # Math Assuming 50-Ohm System impedance relationship
            unit_offsets = {
                "dBuV": 0.0,
                "dBV": -120.0,
                "dBm": -107.0,
                "dBuA": -34.0,   # dBuA = dBuV - 34
                "dBpT": -34.0,   # Placeholder approx, typically requires loop antenna factor
                "dBuV/m": 0.0    # Requires Antenna Factor (.cor)
            }
            conversion_offset = unit_offsets.get(target_unit, 0.0)
            
            # Apply Mathematical Conversion Shift
            final_values += conversion_offset
            
            # Load the locked Limits
